...
class ParquetLoader(Base):
    ...
    def __init__(self, folder_path, system_name):
        ...
        self.subfix_to_remove = ['parquet', 'copy', 'IXH1820', 'iH1000F', 'VIsion' ]

    def get_data(self):
        system_names = []
        for f_name in self.file_names:
            words = [i for i in f_name.replace('.', '-').split('-') if i not in self.subfix_to_remove]
            name = '-'.join(words)
...
class OdbcLoaderDP(Base):
    ...
    @log_time
    def get_data(self,database, system_type_id, system_name, parameter_numberlist=None, start_datetime=datetime.date(1970, 1, 1),
        ...
        if parameter_numberlist is None:
